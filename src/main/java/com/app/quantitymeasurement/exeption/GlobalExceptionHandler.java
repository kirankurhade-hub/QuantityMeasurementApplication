...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<Map<String, Object>> handleUnsupportedOp(UnsupportedOperationException ex,
		...
		return build(HttpStatus.BAD_REQUEST, "Unsupported Operation", ex.getMessage(), request.getRequestURI());
	}

	// ── 4. Catch-all ─────────────────────────────────────────────
	@ExceptionHandler(Exception.class)
	public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex, HttpServletRequest request) {
		logger.error("Unhandled exception [{}] {}: {}", request.getRequestURI(), ex.getClass().getSimpleName(),
				ex.getMessage());
